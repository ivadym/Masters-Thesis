...
% goodwin 2016
% The resulting change in pH is detected by an integrated complementary metal-oxide-semiconductor (CMOS) and an ion-sensitive field-effect transistor (ISFET) (FIG. 4b). The pH change detected by the sensor is imperfectly proportional to the number of nucleotides detected, allowing for limited accuracy in measuring homopolymer lengths.

%chadran
%  If the incubated dNTP matches the growing template strand, the DNA polymerase adds it. The incorporation of dNTP leads to the discharge of a hydrogen ion that activates an ion-sensitive field-effect transistor (ISFET) ion sensor, which can detect the nucleotide by converting the electric signal into base sequences (Fig. 1.4). 
%The remaining nucleotides are washed out and the next cycle continues.
% If repeated bases are present in the template, multiple nucleotides will be added in a single cycle; in this case, a stronger electric signal will be detected based on the hydrogen ion release.

% video
% The release of the hydrogen ion changes the pH of the solution in the well. An ion sensitive layer beneath the well measures the change in pH and convert it to voltage. This voltage change is recorded, indicating the nucleotide was incorporated and the base was called.
%  If the nucleotide is not complementary to the next base, no ion is release and no voltage change is recorded and no base is called. If there are two identical bases next to each other, two nucleotides are incorporated, the voltage doubles and the chip will record identical bases called. 
